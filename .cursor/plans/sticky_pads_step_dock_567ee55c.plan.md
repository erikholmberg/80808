...
overview: Add a desktop-only fixed mini bar at the top of the viewport that duplicates the 16 beat indicators and drum pads when the main TR-808 panel scrolls out of view, using IntersectionObserver and sharedpresentational components with `inert` on the original controls while the dock is visible to avoid duplicate keyboard focus.
todos:
  - id: extract-beat-row
    content: Extract BeatStepIndicatorRow from Tr808Panel; default + compact density
    status: completed
  - id: keyboard-dense-variant
    content: KeyboardMapLegend dense/sticky variant (no title, tighter chips)
    status: completed
  - id: tr808-inert-wrap
    content: "Tr808Panel: padsSectionInert + inert wrapper around steps + legend"
    status: completed
...
- Wrap [`Tr808Panel`](src/components/Tr808Panel.tsx) in a container with `ref`.
...
| [`src/components/Tr808Panel.tsx`](src/components/Tr808Panel.tsx) | **`padsSectionInert`**, wrap steps + legend in **`inert`** div; use **`BeatStepIndicatorRow`** |
