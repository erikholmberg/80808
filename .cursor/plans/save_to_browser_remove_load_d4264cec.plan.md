...
todos:
  - id: drum-machine-save-remove-import
    ...
    status: completed
  - id: tr808-panel
    content: "Tr808Panel: remove Load; rename onSaveFile → onSave; wire new prop"
    status: completed
  - id: saved-panel-copy
...
- **Save** in [`Tr808Panel`](src/components/Tr808Panel.tsx) calls [`onSaveFile`](src/app/components/DrumMachine.tsx) → downloads `pattern` as a `.json` blob.
...
4. **Rename prop** → `onSaveFile` → `onSave` on [`Tr808Panel`](src/components/Tr808Panel.tsx) and pass the updated prop from [`DrumMachine.tsx`](src/app/components/DrumMachine.tsx).
